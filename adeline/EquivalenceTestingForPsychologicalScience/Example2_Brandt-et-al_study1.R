...
rep.test <- t.test(Brandt.data$WellLitSca[Brandt.data$ExpCond==-1],
                   Brandt.data$WellLitSca[Brandt.data$ExpCond==1], 
                   var.equal=FALSE, paired=FALSE)
rep.t <- rep.test$statistic
rep.p <- rep.test$p.value
rep.df <- rep.test$parameter
rep.d <- (rep.m1 - rep.m2)/sqrt(((rep.n1-1)*rep.sd1^2 + 
                                 (rep.n2-1)*rep.sd2^2)/(rep.n1+rep.n2-2))
